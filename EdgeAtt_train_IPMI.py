...
from tools.Niyah_dataset import DADatasetWithoutGT
...
if __name__ == "__main__":
    ...
    tar_img_dir = '/home/niuyi/dataset/UDA/IPMI/image'
    ...
    pred_save_path = './result/EdgeAtt_IPMI_result/{0}'.format(TIME)
    model_save_path = './model/EdgeAtt_IPMI_model/{0}'.format(TIME)

    if not os.path.exists(pred_save_path):
        os.makedirs(pred_save_path)
    if not os.path.exists(model_save_path):
        os.makedirs(model_save_path)
    if not os.path.exists('./logs/EdgeAtt_IPMI_logs/{0}'.format(TIME)):
        os.makedirs('./logs/EdgeAtt_IPMI_logs/{0}'.format(TIME))

    # step 1
    train_set = DADatasetWithoutGT(source_img_dir, source_mask_dir, tar_img_dir, transform=transform_)
    ...
    for epoch in range(opt.n_epochs):
        ...
        for iter, (s_img, s_mask, t_img) in enumerate(train_loader):
            G_TS.train()
            G_ST.train()
            D_T.train()
            D_S.train()
            ...
            real_s, s_mask, real_t = s_img.cuda(), s_mask.cuda(), t_img.cuda()
            ## 全真，全假的标签
            ...
            vutils.save_image(torch.cat([real_t.cpu(), fake_S.cpu(), mask_T.cpu(), t_edge.cpu(), pre_edge_T.ge(opt.mask_thres).cpu()], dim=0), './result/EdgeAtt_IPMI_result/{0}/epoch_{1}.png'.format(TIME, epoch))
            print("Training:Epoch[{:0>3}/{:0>3}] Iteration[{:0>3}/{:0>3}] cycle_loss: {:.4f} edge_loss: {:.4f}  " 
                    "GAN_loss: {:.4f} mask_loss: {:.4f}  G_loss:{:.4f} D_T_loss:{:.4f} D_S_loss:{:.4f} D_mask_loss:{:.4f}".format(epoch, opt.n_epochs, iter + 1, len(train_loader),
                    loss_cycle.detach().cpu().numpy(), loss_edge.detach().cpu().numpy(), loss_GAN.detach().cpu().numpy(), loss_mask.detach().cpu().numpy(),
                    loss_G.detach().cpu().numpy(), loss_D_T.detach().cpu().numpy(), loss_D_S.detach().cpu().numpy(), loss_D_mask.detach().cpu().numpy()))
        ...
        with SummaryWriter('./logs/EdgeAtt_IPMI_logs/{0}'.format(TIME)) as writer:
        ...
        if epoch>=50:
            ...
            torch.save(D_mask.state_dict(), os.path.join(model_save_path, "D_mask_{}_epoch.pkl".format(epoch)))
            torch.save(D_S.state_dict(), os.path.join(model_save_path, "D_S_{}_epoch.pkl".format(epoch)))
            torch.save(D_T.state_dict(), os.path.join(model_save_path, "D_T_{}_epoch.pkl".format(epoch)))
